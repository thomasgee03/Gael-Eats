...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      return <p className="text-red-500 p-8">Something went wrong rendering Black Label Grill.</p>;
    }
    return this.props.children;
  }
}

function BlackLabelGrill() {
  ...
  const fetchSubmissions = async () => {
    ...
    setError('');
    try {
      const res = await axios.get("https://gael-eats-1.onrender.com/api/submissions/black-label-grill");
  ...
  return (
    <ErrorBoundary>
      <div className="p-8">
        <h2 className="text-3xl font-bold">Black Label Grill</h2>
        <p className="text-gray-600">Premium Grilled Delights</p>
        <button
...
export default BlackLabelGrill;
