...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      return <p className="text-red-500 p-8">Something went wrong rendering Clean Plates.</p>;
    }
...
function CleanPlates() {
  ...
  const fetchSubmissions = async () => {
    ...
    setError('');
    try {
      const res = await axios.get("https://gael-eats-1.onrender.com/api/submissions/clean-plates");
  ...
  return (
    <ErrorBoundary>
      <div className="p-8">
        <h2 className="text-3xl font-bold">Clean Plates</h2>
        <p className="text-gray-600">Allergen Friendly Zone</p>
        <button
