...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      return <p className="text-red-500 p-8">Something went wrong rendering Central Oven.</p>;
    }
    return this.props.children;
  }
}

function CentralOven() {
  ...
  const fetchSubmissions = async () => {
    ...
    setError('');
    try {
      const res = await axios.get("https://gael-eats-1.onrender.com/api/submissions/central-oven");
  ...
  return (
    <ErrorBoundary>
      <div className="p-8">
        <h2 className="text-3xl font-bold">Central Oven</h2>
        <p className="text-gray-600">Trattoria</p>
        <button
